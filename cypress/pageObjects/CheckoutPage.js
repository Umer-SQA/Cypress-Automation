export class CheckoutPage
{
    ...
    {
        cy.get(':nth-child(8) > .dropdown-toggle').should('have.attr','href','/sunglasses').click({force:true}) //menu
        cy.get(':nth-child(1) > .mask-wrap > :nth-child(2) > .glass-mask').eq(0).click({force:true}) //first product
    }
    addAProductToCartWithoutProtection()
    {
        cy.get('.col-md-12 > .btn-cart > .btn').should('have.text','Add to Cart').should('be.visible').click({force:true})
    ...
    validateRedeemCodeFunctionality()
    {
        ...
        cy.get('#checkout-total-cost').then((element)=>{
                ...
                cy.get('#promo-code-input').should('have.attr','placeholder','Code').type('RX25') //Add promo code
